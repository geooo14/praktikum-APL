#include <iostream>
#include <iomanip>
#include <stdexcept> // library exception
using namespace std;
...
void tambahRental(Rental *r, int *jumlah){
    try{
        ...
        if((r+*jumlah)->id < 0){
            throw invalid_argument("ID tidak boleh negatif!");
        }
        ...
        if((r+*jumlah)->lamaSewa <= 0 || (r+*jumlah)->harga <= 0){
            throw runtime_error("Lama sewa dan harga harus > 0!");
        }
...
void ubah(Rental *r, int jumlah){
    try{
        ...
        if(idx < 1 || idx > jumlah){
            throw out_of_range("Index tidak valid!");
        }
...
int binarySearchID(Rental *r, int n, int target){
    ...
    while(low <= high){
        ...
        if((r+mid)->id == target) return mid;
        else if((r+mid)->id < target) low = mid + 1;
        else high = mid - 1;
    }
    return -1;
}
                ...
                do{
                    ...
                    if(menu==1) tambahRental(dataRental,&jumlahData);
                        ...
                        if(pilihSearch==1){
                            ...
                            if(hasil != -1)
                                cout<<"Ditemukan: "<<(dataRental+hasil)->namaPenyewa<<endl;
                            else
